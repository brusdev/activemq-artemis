...
final class FileDistributedLock implements DistributedLock {
   ...
   FileDistributedLock(Consumer<String> onClosedLock, File locksFolder, String lockId) {
   ...
   public boolean isHeld() {
      ...
      FileLock dummyLock;
      try {
         dummyLock = channel.tryLock();
      } catch (OverlappingFileLockException held) {
         // this process is holding this lock
         return true;
      } catch (Throwable t) {
         throw new IllegalStateException(t);
      }
      if (dummyLock == null) {
         // another process is holding this
         return true;
      }
      try {
         dummyLock.close();
