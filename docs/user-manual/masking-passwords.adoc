...
By default, the `mask` command uses the legacy two-way algorithm of <<the-default-codec,the default codec>> unless <<using-a-custom-codec,a custom codec>> is defined in `broker.xml` and the `--password-codec` option is `true`. The legacy two-way algorithm is now *deprecated* and exists only to maintain backward-compatibility, use a more secure algorithm by setting the two-way-1 algorithm, for further details see <<the-default-codec,the default codec>>.
