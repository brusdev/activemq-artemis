...
public class MultipleTargetsTest extends ClusterTestBase {
   ...
   public void setup() throws Exception {
      ...
      BrokerBalancerConfiguration brokerBalancerConfiguration = new BrokerBalancerConfiguration()
         .setName(BROKER_BALANCER_NAME).setPolicyConfiguration(new PolicyConfiguration().setName(policyName));
   ...
   @Test
   public void testTargetReadiness1() throws Exception {
      BrokerBalancer balancer = getServer(0).getBalancerManager().getBalancer(BROKER_BALANCER_NAME);

      Wait.assertTrue(() -> balancer.getPool().getTargets().size() > 0, POOL_CHECK_PERIOD * 3, WAIT_PERIOD);
