      ...
      if (withPartition) {
         for (int node = 0; node < 5; node++) {
            ...
            Wait.assertTrue(() -> !((ClusterConnectionImpl)getServer(serverNode).getClusterManager().
                getClusterConnection("cluster" + serverNode)).getTopologyScanner().isBooked());
         }
      }
   ...
   public abstract static class ProxyConnectorFactory implements ConnectorFactory {
      ...
      private class ProxyConnector implements Connector {
         ...
         public Connection createConnection() {
            ...
            if (ProxyConnectorFactory.interceptor == null ||
                ProxyConnectorFactory.interceptor.allowConnection((int)configuration.get("proxyNodeId"), configuration)) {
